...
export default function Paperlink(paper: PaperProps) {
  ...
  return <div>
    ...
    {paper.venue && (
      <span className="ml-2 inline-block max-w-full align-middle font-mono text-xs uppercase tracking-wider text-neutral-500 dark:text-neutral-400 border border-neutral-300 dark:border-neutral-700 px-1.5 py-0.5">{paper.venue}</span>
    )}
    {hasChildren && (
      <div className="ml-6 border-l-2 border-neutral-300 dark:border-neutral-700 pl-2 mt-1 space-y-0.5">
        {paper.awards && paper.awards.map((award, index) => (
          <p key={`award-${index}`} className="text-sm font-medium text-primary-600 dark:text-primary-400">
